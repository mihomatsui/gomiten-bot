...
class GarbageDate
  ...
  # 毎週火・金は可燃ごみの日
    *%w(火 金).product([*1..5]).map { |wday, nth| GarbageDate.new(wday: wday, nth: nth, type: '可燃ゴミ')},
    GarbageDate.new(wday: '水', nth: 3, type: '不燃ゴミ'),
    *%w(水).product([*1..5]).map { |wday, nth| GarbageDate.new(wday: wday, nth: nth, type: 'プラスチックゴミ')},
    *%w(木).product([*1..5]).map { |wday, nth| GarbageDate.new(wday: wday, nth: nth, type: '缶・ビン・ペットボトル')}
  ]
