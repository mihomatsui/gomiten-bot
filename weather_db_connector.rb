...
class WeatherDbConnector
  ...
  def notification_enable_user(user_id)
    p 'enable_user'
    @conn.exec("insert into notifications (user_id, hour, minute, area_id, is_enabled) values ('#{user_id}', #{DEFAULT_WEATHER_HOUR},#{DEFAULT_WEATHER_MINUTE}, #{DEFAULT_AREA_ID}, true) on conflict on constraint notifications_pkey do update set user_id = excluded.user_id, is_enabled = excluded.is_enabled;")
  end

  def notification_disnable_user(user_id)
    p 'disnable_user'
    @conn.exec("insert into notifications (user_id, hour,minute, area_id, is_enabled) values ('#{user_id}', #{DEFAULT_WEATHER_HOUR},#{DEFAULT_WEATHER_MINUTE}, #{DEFAULT_AREA_ID}, false) on conflict on constraint notifications_pkey do update set user_id = excluded.user_id, is_enabled = excluded.is_enabled;")
  end
